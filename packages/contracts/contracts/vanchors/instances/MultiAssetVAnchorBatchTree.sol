...
contract MultiAssetVAnchorBatchTree is MultiAssetVAnchor, ProxiedBatchTree {
	...
	*/
	...
	function _executeInsertions(
		...
		// Queue the output commitments
		IMASPProxy(proxy).queueShieldedTranferCommitment(_publicInputs.outputCommitments);
	}
	...
	function _executeFeeInsertions(
		...
		// Queue the fee output commitments
		IMASPProxy(proxy).queueShieldedTranferCommitment(feeOutputCommitments);
	}
}
