...
abstract contract MerkleTreeWithHistory is MerkleSystem {
    ...
    function getLastRoot() override public view returns (uint256) {
        return roots[currentRootIndex].root;
    }

    /// @inheritdoc IMerkleSystem
    function getZeroHash(uint32 index) override external view returns (uint256) {
    ...
    function _insert(uint256 _leaf) override internal returns (uint32 index) {
    ...
    function _insertTwo(uint256 _leaf1, uint256 _leaf2) override internal returns (uint32 index) {
        uint32 _nextIndex = nextIndex;
        require(_nextIndex != uint32(2)**levels, "Merkle tree is full. No more leaves can be added");
