 ...
 */
abstract contract LinkableAnchorForest is ILinkableAnchor, MerkleForest, ReentrancyGuard, ChainIdWithType, ProposalNonceTracker {
    ...
    */
    ...
    function updateEdge(
        ...
        if (this.hasEdge(_srcChainID)) {
            ...
            edgeList[index].root = uint(_root);
        ...
        for (uint256 i = 0; i < maxEdges; i++) {
            if (edgeList.length >= i + 1) {
                ...
                edges[i] = Edge({
                    ...
                    root: uint(hasher.zeros(forestDepth - 1)),
                    chainID: 0,
                    latestLeafIndex: 0,
                    srcResourceID: 0x0
                });
    ...
    function getLatestNeighborRoots() public view returns (bytes32[] memory) {
        ...
        for (uint256 i = 0; i < maxEdges; i++) {
            if (edgeList.length >= i + 1) {
                roots[i] = bytes32(edgeList[i].root);
            } else {
                // merkle tree height for zeros
                roots[i] = hasher.zeros(forestDepth - 1);
            }
        }
    ...
    function isValidRoots(bytes32[] memory _roots) public view returns (bool) {
        require(isKnownRoot(uint(_roots[0])), "Cannot find your merkle root");
        ...
        while (rootIndex != maxEdges + 1) {
            require(_roots[rootIndex] == hasher.zeros(forestDepth - 1), "non-existent edge is not set to the default root");
            rootIndex++;
        }
        return true;
    }
