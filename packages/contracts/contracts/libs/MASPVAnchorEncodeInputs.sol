...
library VAnchorEncodeInputs {
    ...
    /**
        ...
        @param publicAssetID The public asset ID registered on the Tangle Network
        @param publicTokenID The token identifier -- non-zero only for NFTs
    ...
    */
    ...
    function _encodeInputs2(
        ...
        if (_maxEdges == 1) {
            ...
            inputs[2] = uint256(_args.publicAssetID);
            inputs[3] = uint256(_args.publicTokenID);
            ...
            bytes32[7] memory roots = abi.decode(_args.roots, (bytes32[7]));
            // assign roots
            result[0] = roots[0];
            result[1] = roots[1];
            result[2] = roots[2];
