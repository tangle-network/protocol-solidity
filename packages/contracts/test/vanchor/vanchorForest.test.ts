...
describe.skip('VAnchorForest for 1 max edge', () => {
  ...
  describe('#transact', () => {
    ...
    it('should be compliant', async function () {
      ...
      // it's the same as merklePath and merklePathIndexes and index in the tree
      const index = tree.indexOf(toFixedHex(BigInt(Buffer.from(commitment).toString('hex'))));
    ...
    it('should reject proofs made against roots of empty edges', async () => {
      ...
      const extAmount =
        ...
        fakeInputs.reduce((sum, x) => sum + BigInt(x.amount), BigInt(0));
      ...
      let forestPathElements = [
        fakeForestProof.pathElements.map((el) => el.toString()),
        emptyMerkleProof.forestPathElements.map((el: BigNumberish) => el.toString()),
      ];
      let subtreeIndices = [
      ...
      const proofInput = {
        ...
        inputNullifier: vanchorInput.inputNullifier.map((hexStr) => BigInt(hexStr).toString()),
    ...
    describe('#cross-chain test', () => {
      ...
      it('should cross chain withdraw', async () => {
        ...
        const subtreeLeaves = ganacheAnchor.tree
          .elements()
          .map((el: BigNumberish) => el.toString(16));
