...
```solidity
function transact(IdentityVAnchorEncodeInputs.Proof _args, ExtData _extData) external nonpayable
```
...
| Name | Type | Description |
|---|---|---|
| _args | IdentityVAnchorEncodeInputs.Proof | The zkSNARK proof parameters
| _extData | ExtData | The external data for the transaction

### transactWrap

```solidity
function transactWrap(IdentityVAnchorEncodeInputs.Proof _args, ExtData _extData, address _tokenAddress) external payable
```
