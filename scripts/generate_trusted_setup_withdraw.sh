...
if [ ! -f $OUTPUT_DIR/pot12_0001.ptau ]; then
...
# Apply random beacon
if [ ! -f $OUTPUT_DIR/pot12_beacon.ptau ]; then
...
# Prapare phase 2
if [ ! -f $OUTPUT_DIR/pot12_final.ptau ]; then
    echo "snarkjs powersoftau prepare phase2 $OUTPUT_DIR/pot12_0001.ptau pot12_final.ptau -v\n"
    snarkjs powersoftau prepare phase2 $OUTPUT_DIR/pot12_0001.ptau $OUTPUT_DIR/pot12_final.ptau -v
    wait 500
fi
...
if [ ! -f $OUTPUT_DIR/circuit_0000.zkey ]; then
    ...
    snarkjs groth16 setup "${R1CS_PATH}" $OUTPUT_DIR/pot12_final.ptau $OUTPUT_DIR/circuit_0000.zkey
    wait 500
fi
...
snarkjs zkey verify $R1CS_PATH $OUTPUT_DIR/pot12_final.ptau $OUTPUT_DIR/circuit_0001.zkey
wait 500
...
snarkjs zkey verify $R1CS_PATH $OUTPUT_DIR/pot12_final.ptau $OUTPUT_DIR/circuit_final.zkey
wait 500
