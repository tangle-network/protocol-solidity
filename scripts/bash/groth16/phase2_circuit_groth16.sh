compile_phase2 () {
    ...
    npx snarkjs groth16 setup "$pathToCircuitDir/$circuit.r1cs" ./protocol-solidity-fixtures/ptau/pot16_final.ptau "$outdir/circuit_0000.zkey"
    echo "test" | npx snarkjs zkey contribute "$outdir/circuit_0000.zkey" "$outdir/circuit_0001.zkey" --name"1st Contributor name" -v
    npx snarkjs zkey verify "$pathToCircuitDir/$circuit.r1cs" ./protocol-solidity-fixtures/ptau/pot16_final.ptau "$outdir/circuit_0001.zkey"
    ...
    npx snarkjs zkey verify "$pathToCircuitDir/$circuit.r1cs" ./protocol-solidity-fixtures/ptau/pot16_final.ptau "$outdir/circuit_final.zkey"
...
move_verifiers_and_metadata () {
    ...
    cp $1/circuit_final.zkey protocol-solidity-fixtures/fixtures/$anchorType/$size/circuit_final.zkey
    cp $1/verifier.sol contracts/verifiers/$anchorType/"Verifier$size.sol"
