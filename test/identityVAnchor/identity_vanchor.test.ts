...
describe('IdentityVAnchor for 2 max edges', () => {
  ...
  beforeEach(async () => {
    ...
    // create Anchor
    anchor = await IdentityVAnchor.createIdentityVAnchor(
  ...
  describe('#transact', () => {
    ...
    it('Should reject proofs made against roots of empty edges', async () => {
      ...
      const setupIdentityVAnchor = new SetupTxIdentityVAnchorMock(anchor.contract, anchor.signer, 30, 1, anchor.smallCircuitZkComponents, anchor.largeCircuitZkComponents, roots);
      setupIdentityVAnchor.token = anchor.token;
      let inputs: Utxo[] = [
        fakeUtxo,
      ...
      const {publicInputs, extData} = await setupIdentityVAnchor.setupTransaction(
        inputs,
        outputs,
        extAmount,
        0,
  ...
  describe('#wrapping tests', () => {
    it('should wrap and deposit', async () => {
      ...
      const wrappedAnchor = await IdentityVAnchor.createIdentityVAnchor(
        verifier.contract.address,
        30,
        hasherInstance.address,
        sender.address,
    ...
    it('should withdraw and unwrap', async () => {
      ...
      let anchorLeaves = wrappedIdentityVAnchor.tree.elements().map((leaf) => hexToU8a(leaf.toHexString()));
    ...
    it('wrapping fee should work correctly with transactWrap', async () => {
      ...
      const balWrappedTokenBeforeDepositAnchor = await wrappedToken.balanceOf(wrappedIdentityVAnchor.contract.address);
      ...
      const balWrappedTokenAfterDepositAnchor = await wrappedToken.balanceOf(wrappedIdentityVAnchor.contract.address);
      ...
      anchorLeaves = wrappedIdentityVAnchor.tree.elements().map((leaf) => hexToU8a(leaf.toHexString()));
      ...
      const balWrappedTokenAfterWithdrawAnchor = await wrappedToken.balanceOf(wrappedIdentityVAnchor.contract.address);
