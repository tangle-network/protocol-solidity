...
describe('IdentityVAnchor for 2 max edges', () => {
  ...
  describe('#transact', () => {
    it('alice should deposit', async () => {
      ...
      // , await generateUTXOForTest(chainID, new Keypair())];
      const res = await idAnchor.transact(aliceKeypair, inputs, outputs, fee, BigNumber.from(0), recipient, relayer);
      expect(res.tx)
    ...
    it('should be able to transfer to bob', async function () {
      ...
      const outputs = [aliceTransferUtxo];
      ...
      // const withdrawInputs = spendableUtxos;
      // console.log("SPENDABLE: ", spendableUtxos)
    ...
    const identityRootInputs = idAnchor.populateIdentityRootsForProof();
    const identityMerkleProof: MerkleProof = idAnchor.generateIdentityMerkleProof(bobKeypair.getPubKey());
    ...
    const outSemaphoreProofs = withdrawOutputs.map((utxo) => {
      const leaf = utxo.keypair.getPubKey();
      if (Number(utxo.amount) > 0) {
        const idx = group.indexOf(leaf);
        return group.generateProofOfMembership(idx);
    ...
    let extAmount = BigNumber.from(fee)
      .add(withdrawOutputs.reduce((sum, x) => sum.add(BigNumber.from(BigInt(x.amount))), BigNumber.from(0)))
      .sub(spendableUtxos.reduce((sum, x) => sum.add(BigNumber.from(BigInt(x.amount))), BigNumber.from(0)));
    const vanchorMerkleProof = spendableUtxos.map((x) => idAnchor.getMerkleProof(x));

    const encOutput1 = withdrawOutputs[0].encrypt();
    const encOutput2 = withdrawOutputs[1].encrypt();
    const extData = {
      recipient: toFixedHex(recipient.address, 20),
      extAmount: toFixedHex(extAmount),
      relayer: toFixedHex(relayer, 20),
      fee: toFixedHex(fee),
      refund: toFixedHex(BigNumber.from(0).toString()),
    ...
    const withdrawExtDataHash = await getVAnchorExtDataHash(
      encOutput1,
      encOutput2,
      extAmount.toString(),
      BigNumber.from(fee).toString(),
    ...
    it('should spend input utxo and create output utxo', async () => {
      ...
      const aliceDepositUtxo = await generateUTXOForTest(chainID, aliceKeypair, aliceDepositAmount);
      // , await generateUTXOForTest(chainID, new Keypair())];
      const res = await idAnchor.transact(aliceKeypair, [], [aliceDepositUtxo], fee, BigNumber.from(0), recipient, relayer);
      expect(res.tx)
        .to.emit(idAnchor.contract, 'NewCommitment')
      ...
      const inputs: Utxo[] = [aliceDepositUtxo];
      const outputs = [aliceRefreshUtxo];

      const otherRes = await idAnchor.transact(aliceKeypair, inputs, outputs, fee, BigNumber.from(0), recipient, relayer);
