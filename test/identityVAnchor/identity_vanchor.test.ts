...
describe('IdentityVAnchor for 2 max edges', () => {
  ...
  describe('#transact', () => {
    ...
    it('should be able to transfer to bob', async function () {
      ...
      // pubkeys are stored on semaphore-groups contract openly so we can assume to know bobPubKey
      const bobLeaf = bobKeypair.getPubKey()
      const aliceLeaf = aliceKeypair.getPubKey()
      const bobIndex = group.indexOf(bobLeaf)
      const bobMerkleProof = group.generateProofOfMembership(bobIndex)
