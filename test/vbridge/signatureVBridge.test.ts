 ...
 describe('multichain tests for vbridge', () => {
   ...
   describe('BridgeConstruction', () => {
     ...
     it('create 2 side bridge for one token', async () => {
       ...
       const vBridge = await VBridge.deployVBridge(bridge2WebbEthInput, deploymentConfig, false);
   ...
   describe('2 sided bridge existing token use', () => {
     ...
     beforeEach(async () => {
       ...
       // deploy the bridge
       vBridge = await VBridge.deployVBridge(vBridgeInput, deploymentConfig, false);
