...
import { VAnchorV2 } from '../../packages/anchors/src';
...
describe('VAnchor for 2 max edges', () => {
  ...
  beforeEach(async () => {
    ...
    // create Anchor
    anchor = await VAnchorV2.createVAnchor(
  ...
  describe('#transact', () => {
    ...
    it('should reject tampering with public inputs', async () => {
      ...
      let publicInputArgs:[string, string, string[], [any, any], string, string] = [
        `0x${proofEncoded}`,
        VAnchorV2.createRootsBytes(input.roots.map((x) => x.toString())),
      ...
      let incorrectPublicInputs = VAnchorV2.convertToPublicInputsStruct(incorrectPublicInputArgs);
      let extAmountInputs = VAnchorV2.convertToExtDataStruct(extDataArgs)
      ...
      incorrectPublicInputs = VAnchorV2.convertToPublicInputsStruct(incorrectPublicInputArgs);
      ...
      let correctPublicInputs = VAnchorV2.convertToPublicInputsStruct(publicInputArgs);
      let incorrectExtAmountInputs = VAnchorV2.convertToExtDataStruct(incorrectExtDataArgs)
      ...
      incorrectExtAmountInputs = VAnchorV2.convertToExtDataStruct(incorrectExtDataArgs)
  ...
  describe('#wrapping tests', () => {
    it('should wrap and deposit', async () => {
      ...
      const wrappedAnchor = await VAnchorV2.createVAnchor(
        verifier.contract.address,
        30,
        hasherInstance.address,
        sender.address,
    ...
    it('should withdraw and unwrap', async () => {
      ...
      // create Anchor for wrapped token
      const wrappedVAnchor = await VAnchorV2.createVAnchor(
        verifier.contract.address,
        30,
        hasherInstance.address,
        sender.address,
