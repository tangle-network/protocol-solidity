...
contract('E2E LinkableAnchors - Cross chain withdraw using historical root should work', async accounts => {
  ...
  it('withdrawing across bridge after two deposits should work', async () => {
    ...
    */
    ...
    const firstWithdrawlMerkleRoot = await OriginChainAnchorInstance.getLastRoot();
    ...
    */
    ...
    originUpdateNonce = latestLeafIndex;
    const secondWithdrawalMerkleRoot = await OriginChainAnchorInstance.getLastRoot();
    // create correct update proposal data for the deposit on origin chain
    ...
    */
    ...
    const isSpent = await DestChainAnchorInstance.isSpent(helpers.toFixedHex(input.nullifierHash));
    assert(isSpent);
    /*
    ...
    */
    ...
    let root;
    let result = await tree.path(1);
    ({ root, pathElements, pathIndices } = result);
