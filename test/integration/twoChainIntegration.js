...
contract('E2E LinkableAnchors - Two EVM Chains', async accounts => {
  ...
  const initialBalanceToMint = 10000000000000;
  const maxRoots = 1;
  ...
  let destChainToken;
  let tokenDenomination = BigInt(1e21);
  let tree;
  let createWitness;
  ...
  beforeEach(async () => {
    ...
    // mint initial balances for sender
    await originChainToken.mint(sender, initialBalanceToMint);
    await destChainToken.mint(sender, initialBalanceToMint);
    // increase allowance and set resources for bridge
    await Promise.all([
      originChainToken.approve(OriginChainLinkableAnchorInstance.address, initialBalanceToMint, { from: sender }),
      destChainToken.approve(DestChainLinkableAnchorInstance.address, initialBalanceToMint, { from: sender }),
    ...
    // deposit on both chains and define nonces based on events emitted
