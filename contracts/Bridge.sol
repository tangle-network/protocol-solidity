...
contract Bridge is Pausable, AccessControl, SafeMath {
    ...
    // destinationChainID => number of updates
    mapping(uint8 => uint64) public _updateCounts;
    // resourceID => handler address
    mapping(bytes32 => address) public _resourceIDToAnchorHandlerAddress;
    // destinationChainID + updateNonce => dataHash => Proposal
    mapping(uint72 => mapping(bytes32 => Proposal)) private _anchorProposals;
    ...
    function update(uint8 destinationChainID, bytes32 resourceID, bytes calldata data) external payable whenNotPaused {
        require(msg.value == _fee, "Incorrect fee supplied");

        address handler = _resourceIDToHandlerAddress[resourceID];
        require(handler != address(0), "resourceID not mapped to handler");

        uint64 updateNonce = ++_updateCounts[destinationChainID];
     ...
     */
    function executeProposal(uint8 chainID, uint64 nonce, bytes calldata data, bytes32 resourceID) external onlyRelayers whenNotPaused {
        address handler = _resourceIDToHandlerAddress[resourceID];
